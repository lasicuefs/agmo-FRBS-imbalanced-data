...
public class ArrayReal extends Variable {
  ...
  public ArrayReal(int sizePontos, List<Double> LimitesInferiores, List<Double> LimitesSuperior) {
        size_ = sizePontos;
        array_ = new Double[size_];

        for (int i = 0; i < size_; i++) {
              array_[i] = PseudoRandom.randDouble() * 
                      (LimitesSuperior.get(i) - LimitesInferiores.get(i)) + LimitesInferiores.get(i);
        }
   } // for    }
  public ArrayReal(int size, Problem problem) {
    ...
    for (int i = 0; i < size_; i++) {
      array_[i] = PseudoRandom.randDouble() * 
              (problem_.getUpperLimit(i) - problem_.getLowerLimit(i)) + problem_.getLowerLimit(i);
    } // for
  } // Constructor
  
  
  public ArrayReal(int size, Problem problem, List<Double> cromossomo) {
    problem_ = problem;
    size_ = size;
    array_ = new Double[size_];
    for (int i = 0; i < size_; i++) {
      array_[i] = cromossomo.get(i);
    } // for
  } // Constructor
