...
public class ArrayInt extends Variable {
  ...
  private ArrayInt(ArrayInt arrayInt) {
    ...
    public ArrayInt(int sizeAntecedente, List<Integer> limInfAnt, List<Integer> limSupAnt) {
            size_   = sizeAntecedente;
		array_ = new int[size_];
    ...
    for (int i = 0; i < size_ ; i++) {
    	lowerBounds_[i] = (int)limInfAnt.get(i) ;
    	upperBounds_[i] = (int)limSupAnt.get(i) ;
    	array_[i] = PseudoRandom.randInt(lowerBounds_[i], upperBounds_[i]) ;
    } // for
  } // Constructor
